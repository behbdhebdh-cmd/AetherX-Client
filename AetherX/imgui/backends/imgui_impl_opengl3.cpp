...
{
    const bool needs_upload = (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates);
    ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();

    // Minecraft may leave pixel unpack state configured for its own texture streaming.
    // Reset it around ImGui texture uploads or font atlas glyphs can be read from
    // the wrong offset/stride, which shows up as random missing letters.
    ...
    if (needs_upload)
    {
        ...
        GL_CALL(glGetIntegerv(GL_UNPACK_SKIP_PIXELS, &last_unpack_skip_pixels));
        GL_CALL(glGetIntegerv(GL_UNPACK_SKIP_ROWS, &last_unpack_skip_rows));
        GL_CALL(glPixelStorei(GL_UNPACK_ROW_LENGTH, 0));
        GL_CALL(glPixelStorei(GL_UNPACK_SKIP_PIXELS, 0));
        GL_CALL(glPixelStorei(GL_UNPACK_SKIP_ROWS, 0));
#endif
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_BIND_BUFFER_PIXEL_UNPACK
        if (bd != nullptr && bd->GlVersion >= 210)
        {
    ...
    if (needs_upload)
    {
        ...
        GL_CALL(glPixelStorei(GL_UNPACK_SKIP_PIXELS, last_unpack_skip_pixels));
        GL_CALL(glPixelStorei(GL_UNPACK_SKIP_ROWS, last_unpack_skip_rows));
#endif
